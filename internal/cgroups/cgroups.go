...
//nolint:gochecknoinits // we want to compute it at package initialization
func init() {
	var err error
	cgroupRootPath, err = getHostCgroupRoot()
	if err != nil {
		panic(fmt.Errorf("failed to get host cgroup root: %w", err))
	}
}
