		...
		AddFunc: func(obj interface{}) {
			...
			for containerName, containerRules := range wp.Spec.RulesByContainer {
				...
				if err := p.policyValuesFunc(polID, containerRules.Executables.Allowed, bpf.AddValuesToPolicy); err != nil {
					...
					p.logger.Error(
						"failed to populate policy values",
						"policyID", polID,
						"wp", wp.Name,
				...
				if err := p.policyModeUpdateFunc(polID, bpfMode); err != nil {
					p.logger.Error(
						"failed to set policy mode",
						"mode", bpfMode.String(),
						"policyID", polID,
						"wp", wp.Name,
						"container", containerName,
				...
				if err := p.resolver.AddPolicy(polID, wp.Namespace, podSelector, containerSelector); err != nil {
					p.logger.Error(
						"failed to add policy to resolver",
						"policyID", polID,
						"wp", wp.Name,
						"container", containerName,
						"error", err,
		...
		DeleteFunc: func(obj interface{}) {
			...
			for containerName, policyID := range state {
				if err := p.policyValuesFunc(policyID, []string{}, bpf.RemoveValuesFromPolicy); err != nil {
					p.logger.Error(
						"failed to remove policy values",
						"policyID", policyID,
						"wp", wp.Name,
						"container", containerName,
						"error", err,
				...
				if err := p.resolver.DeletePolicy(policyID); err != nil {
					p.logger.Error("failed to remove policy from cgroup map",
						"policyID", policyID,
						"wp", wp.Name,
						"container", containerName,
						"error", err,
