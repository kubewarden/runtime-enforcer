...
func podForAllContainers(pod *corev1.Pod, fn func(c *corev1.ContainerStatus)) {
...
func podContainersInfoWithoutCgroups(pod *corev1.Pod) map[resolver.ContainerID]*resolver.ContainerData {
	ret := make(map[resolver.ContainerID]*resolver.ContainerData)

	podForAllContainers(pod, func(c *corev1.ContainerStatus) {
		id := containerIDFromContainerStatus(c)
		ret[id] = &resolver.ContainerData{
			CgroupID: 0, // to be populated later
			Name:     c.Name,
		}
	})
	return ret
...
func getBasePodData(pod *corev1.Pod) *resolver.PodData {
	info := &resolver.PodData{
		UID:              string(pod.UID),
		Namespace:        pod.Namespace,
		Name:             pod.Name,
		WorkloadName:     pod.Name,
		WorkloadType:     workloadTypePod,
		PolicyLabelValue: pod.Labels[v1alpha1.PolicyLabelKey],
	}
